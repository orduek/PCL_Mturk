---
title: "Analysis of Prolific & MTurk Data - PCL > 28"
...
# read data
prolific_cleaned <- read_csv("df_combined.csv")
mTurk_cleaned <- read_csv("mTurk_cleaned.csv")

mTurk_cleaned <- mTurk_cleaned %>%
  select(-totalPCL) %>% 
  mutate(dich = case_when(
    dich == 2 ~ "Fearful Reaction",
    dich == 1 ~ "Emotional Pain",
    TRUE ~ as.character(dich)
  ))  %>%
    rename(PCL.Total = `PCL Total`)  

df <- prolific_cleaned# bind_rows(prolific_cleaned,mTurk_cleaned)
```
...
df_1 <- df %>% 
  rename(emo_pain_single = pain,
    ...
    mutate(DMS_5_Reexp = PCL1 + PCL2 + PCL3 + PCL4 + PCL5,
         ...
         DMS_5_Negative_Cognitions_minus_11 = PCL8 + PCL9 + PCL10 + PCL11 + PCL12 + PCL13 + PCL14)  %>% filter(totalPCL > 28)
```
...
data_table1_descriptive %>%
  select(age, sex, education,emo_pain_single, fear_single, dich, totalPCL,DMS_5_Reexp, totalRTQ,  totalGAD, totalASI) %>% 
  tbl_summary(by=dich) %>% 
...
#### Remove emotional pain - just shame and fear

```{r PCL 11 factors NW, message=FALSE, warning=FALSE, include=FALSE}
df_network_PTSD_11_clusters2 <- df_1 %>% 
  select(DMS_5_Reexp, DMS_5_Avoidance, DMS_5_Avoidance, 
         DMS_5_Negative_Cognitions_minus_11, DMS_5_Arousal,
         PCL_fear_horror, PCL_shame_guilt,  fear_single) %>% 
    rename(
        Fear = fear_single,
        Horror = PCL_fear_horror,
        Shame = PCL_shame_guilt,
...
NW_External_PTSD_11_clusters2 <- estimateNetwork(df_network_PTSD_11_clusters2, "ggmModSelect")
```

```{r PCL 11 factors NW plot, echo=FALSE, message=FALSE, warning=FALSE}
plot(NW_External_PTSD_11_clusters2, layout = "circle",
     title = "Associations with PTSD DSM Factors, Horror & Shame")
```
...
### 8-factor with shame and not emotional pain
```{r Full Factors NW, message=FALSE, warning=FALSE, include=FALSE}
### Clusters
df_network_full_clusters2 <- df_1 %>% 
  select(RE1:Da, 
         PCL_fear_horror, PCL_shame_guilt, 
         totalRTQ,  
          fear_single) %>% 
...
NW_External_full_clusters2 <- estimateNetwork(df_network_full_clusters2, "ggmModSelect")
```

```{r Full Factors NW plot, echo=FALSE, message=FALSE, warning=FALSE}
plot(NW_External_full_clusters2, layout = "circle")
```
...
ggplot(df_3, aes(x = emo_pain_single, y = fear_single, color=dich)) +
...
## Multi-item basis
We create a score for Emotional Pain and Fear based on items that were uniquely related to the two items. This should increase reliability. First, we create the score.
...
df_screening_tool <- df_1 %>% 
  mutate(screening_score_fear = PCL1 + PCL2 + PCL3 + PCL7 + PCL_fear_horror,
         screening_score_emo_pain_single = PCL12 + PCL13 + PCL14 + totalRTQ) %>% mutate(across(starts_with("screening_score"), scale))
# I have removed the scaling for ease of computing median etc. But, for plotting porpuses, scaling is preffered # totalRTQ
...
# Create a scatter plot with a correlation line
ggplot(df_screening_tool, aes(x=screening_score_emo_pain_single, y=screening_score_fear, color=dich)) +
  geom_point() +
  geom_jitter(width = 0.2, height = 0.2) +
  geom_smooth(method=lm, se=FALSE, color="red") +
  theme_minimal() +
...
Now we use the same categorical table to show how many are in each section
```{r new screening tool Ns}
# Calculate median of 'emo_pain_single'
median_emo_pain_srceen <- median(df_screening_tool$screening_score_emo_pain_single, na.rm = TRUE)
median_fear_screen <- median(df_screening_tool$screening_score_fear, na.rm = TRUE)

df_screening_tool_2 <- df_screening_tool %>%
  mutate(emo_pain_screen_median = ifelse(screening_score_emo_pain_single >= median_emo_pain_srceen, 1, 0),
         fear_screen_median = ifelse(screening_score_fear >= median_fear_screen, 1, 0))

df_screening_tool_2 <- df_screening_tool_2 %>% 
  mutate(pain_fear_category_screen = case_when(
    emo_pain_screen_median == 1 & fear_screen_median == 1 ~ "High Pain and High Fear",
    emo_pain_screen_median == 1 & fear_screen_median == 0 ~ "High Pain and Low Fear",
    emo_pain_screen_median == 0 & fear_screen_median == 1 ~ "Low Pain and High Fear",
    emo_pain_screen_median == 0 & fear_screen_median == 0 ~ "Low Pain and Low Fear",
  ))

# Count the number of subjects in each category
category_counts <- table(df_screening_tool_2$pain_fear_category_screen)
category_counts   
...
```{r}
shame_scale <- scale(df_1$PCL_shame_guilt)
ggplot(df_1, aes(x =PCL_shame_guilt, y = fear_single, color= dich)) +
  geom_jitter(width = 0.2, height = 0.2) +
  geom_smooth(method = lm, se = FALSE, color = "red") +
  theme_minimal() +
  labs(title = "Scatter plot of standartized Shame vs Fear", x="Shame", y="Fear", x = "Shame", y = "fear_single")


cor.test(df_1$PCL_shame_guilt, df_1$fear_single)
```


## Using Shame instead of emotional pain
### Network analysis (8-factor)


```{r 8-Factor Model NW, message=FALSE, warning=FALSE, include=FALSE}
df_network_PTSD_8_factorS <- df_1 %>% 
  select(RE1:Da, fear_single, PCL_shame_guilt) %>% 
  rename(Fear = fear_single,
         Shame = PCL_shame_guilt)

NW_PTSD_8_factorS <- estimateNetwork(df_network_PTSD_8_factorS, "ggmModSelect")
```

```{r 8-Factor Model NW plot, echo=FALSE, message=FALSE, warning=FALSE}
plot(NW_PTSD_8_factorS, layout = "circle",
     title = "Associations with PTSD 8 factor model Shame")
```
### Building shame score
...
df_screening_tool <- df_1 %>% 
  mutate(screening_score_fear = PCL1 + PCL2 + PCL3 + PCL7 + PCL_fear_horror,
         screening_score_shame = Na + PCL_shame_guilt + PCL4 + PCL5) %>% mutate(across(c(screening_score_fear, screening_score_shame), ~as.vector(scale(.))))
...
```{r Multi item score plot, echo=FALSE, message=FALSE, warning=FALSE}
# Plotting distributions
p1 <- ggplot(df_screening_tool, aes(x=screening_score_shame)) +
  geom_histogram(bins = 30, fill="blue", alpha = 0.5) +
  theme_minimal() +
  labs(title="Distribution of Shame screening score", x="Severity", y="Frequency")
...
```{r Screening scores correlation, echo=FALSE, warning=FALSE}
correlation_sc <- cor.test(df_screening_tool$screening_score_shame, df_screening_tool$screening_score_fear, method = "spearman")
correlation_sc
```

```{r Screening scores scatter plot, echo=FALSE, warning=FALSE}
# Create a scatter plot with a correlation line
ggplot(df_screening_tool, aes(x=screening_score_shame, y=screening_score_fear)) +
  geom_point() +
  geom_jitter(width = 0.2, height = 0.2) +
  geom_smooth(method=lm, se=FALSE, color="red") +
  theme_minimal() +
  labs(title="Scatter plot of emo_pain_single vs fear", x="shame", y="fear")+
  geom_vline(xintercept = 0, size = 1.5) +
  geom_hline(yintercept = 0, size = 1.5) 
```
Counting categories
...
```{r new screening tool Ns}
# Calculate median of 'emo_pain_single'
median_shame_srceen <- median(df_screening_tool$screening_score_shame, na.rm = TRUE)
median_fear_screen <- median(df_screening_tool$screening_score_fear, na.rm = TRUE)

df_screening_tool_2 <- df_screening_tool %>%
  mutate(shame_screen_median = ifelse(screening_score_shame >= median_shame_srceen, 1, 0),
         fear_screen_median = ifelse(screening_score_fear >= median_fear_screen, 1, 0))


df_screening_tool_2 <- df_screening_tool_2 %>% 
  mutate(pain_fear_category_screen = case_when(
    shame_screen_median == 1 & fear_screen_median == 1 ~ "High Pain and High Fear",
    shame_screen_median == 1 & fear_screen_median == 0 ~ "High Pain and Low Fear",
    shame_screen_median == 0 & fear_screen_median == 1 ~ "Low Pain and High Fear",
    shame_screen_median == 0 & fear_screen_median == 0 ~ "Low Pain and Low Fear",
  ))
# Count the number of subjects in each category
category_counts <- table(df_screening_tool_2$pain_fear_category_screen)
category_counts   
...
### Choose the ones at least 0.5 SE from the middle
```{r}
df_screening_tool_3 <- df_screening_tool_2 %>% 
  mutate(pain_fear_category_screen = case_when(
    screening_score_shame > 0.5 & screening_score_fear > 0.5 ~ "High Pain and High Fear",
    screening_score_shame > 0.5 & screening_score_fear < -0.5 ~ "High Pain and Low Fear",
    screening_score_shame < -0.5 & screening_score_fear > 0.5 ~ "Low Pain and High Fear",
    screening_score_shame < -0.5 & screening_score_fear < -0.5 ~ "Low Pain and Low Fear",
  ))

# Count the number of subjects in each category
category_counts <- table(df_screening_tool_3$pain_fear_category_screen)
category_counts
```

## Network analysis with "new PCL"
Removing items 10 and 11 and adding Q42-Q46
...
df_new <- df %>% 
  rename(emo_pain_single = pain,
         ...
         PCL_blameSelf = Q45,
         PCL_blameElse = Q46) %>% 
  select(PCL1:PCL20,PCL_fear_horror,PCL_shame_guilt,PCL_anger, PCL_blameSelf,PCL_blameElse, -PCL10, -PCL11) %>% drop_na()
```

### Estimate network without filtering (i.e., all pcl scores)
```{r}
NW_PTSD_newPCL <- estimateNetwork(df_new, "ggmModSelect")
```
...
plot(NW_PTSD_newPCL, 
     title = "New PCL")
```
### Estimate network with PCL=28 cutoff
...
df_newCut <-  df %>% 
  rename(emo_pain_single = pain,
         ...
         PCL_anger = Q44,
         PCL_blameSelf = Q45,
         PCL_blameElse = Q46) %>% filter(totalPCL > 28) %>% 
  select(PCL1:PCL20,PCL_fear_horror,PCL_shame_guilt,PCL_anger, PCL_blameSelf,PCL_blameElse, -PCL10, -PCL11) %>% drop_na()
```

Network
```{r}
NW_PTSD_newPCL_cutOff <- estimateNetwork(df_newCut, "ggmModSelect")
```

```{r 8-Factor Model NW plot, echo=FALSE, message=FALSE, warning=FALSE}
plot(NW_PTSD_newPCL_cutOff, 
     title = "New PCL")
```
